...
from failing_loudly.shift_reductor import *
from failing_loudly.shared_utils import *
from failing_loudly.data_utils import random_set_features_to_uniform
...
class ShiftDetector:
    ...
    def __init__(self, dr_techniques, test_types, od_tests, md_tests, sign_level, red_models, sample, datset, quality=1.0):
    ...
    def classify_data(self, X_tr, y_tr, X_val, y_val, X_te, y_te, orig_dims, nb_classes, dr_technique=DimensionalityReduction.BBSDh):
        shift_reductor = ShiftReductor(X_tr, y_tr, X_val, y_val, DimensionalityReduction(dr_technique.value), orig_dims, self.datset)
        shift_reductor_model = shift_reductor.fit_reductor()
        X_te_red = shift_reductor.reduce(shift_reductor_model, X_te)
        return X_te_red

    def detect_data_shift(self, X_tr, y_tr, X_val, y_val, X_te, y_te, orig_dims, nb_classes, return_scaled_p_val=True):
        ...
        for dr_ind, dr_technique in enumerate(self.dr_techniques):
            ...
            if (dr_technique == DimensionalityReduction.BBSDs.value or dr_technique == DimensionalityReduction.BBSDs_RF.value) and self.quality < 1.:
                X_tr_red = random_set_features_to_uniform(X_tr_red, corruption_probability=1 - self.quality)
                X_te_red = random_set_features_to_uniform(X_te_red, corruption_probability=1 - self.quality)

            # Compute classification accuracy on both sets for malignancy detection.
            if dr_technique == DimensionalityReduction.BBSDh.value or dr_technique == DimensionalityReduction.BBSDh_RF.value:
                val_acc = np.sum(np.equal(np.squeeze(X_tr_red), y_val).astype(int)) / X_tr_red.shape[0]
                te_acc = np.sum(np.equal(np.squeeze(X_te_red), y_te).astype(int)) / X_te_red.shape[0]
            elif dr_technique == DimensionalityReduction.BBSDs.value or dr_technique == DimensionalityReduction.BBSDs_RF.value:
                pred_tr = np.argmax(X_tr_red, axis=1)
                val_acc = np.sum(np.equal(pred_tr, y_val).astype(int)) / X_tr_red.shape[0]
                pred_te = np.argmax(X_te_red, axis=1)
                te_acc = np.sum(np.equal(pred_te, y_te).astype(int)) / X_te_red.shape[0]
            ...
            if dr_technique != DimensionalityReduction.BBSDh.value and return_scaled_p_val:
        ...
        md_decs, ind_md_decs, ind_md_p_vals), red_dim, self.red_models, val_acc, te_acc
